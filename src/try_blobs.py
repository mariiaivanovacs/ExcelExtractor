...
def process_directory(mode):
    """
    Process a directory of images depending on mode:
      - 'blobs' → from cells_production → call make_blobs()
      - 'words' → from blobs_2 → call make_words()
    """
    if mode == "blobs":
        working_directory = "cells_cleaned"
        process_function = make_blobs
        
    elif mode == "words":
        working_directory = "blobs"
        process_function = make_words
    else:
        print(f"❌ Invalid mode: {mode}. Must be 'blobs' or 'words'.")
        sys.exit(1)

    if not os.path.exists(working_directory):
        print(f"❌ Directory not found: {working_directory}")
        sys.exit(1)

    print(f"\n🚀 Running in '{mode}' mode. Reading from: {working_directory}\n")
    ...
    for filename in os.listdir(working_directory):
        if filename.lower().endswith('.png'):
            ...
            process_function(img, base_name)  # Either make_blobs or make_words
            files_explored.append(filename)

    print(f"\n✅ Done. Processed {len(files_explored)} images from '{working_directory}'.\n")
