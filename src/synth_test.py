...
DEFAULT_WIDTHS = [24, 48, 72, 96, 120, 144, 168, 192, 216, 240, 264, 288, 312, 336, 360, 384, 408, 432, 456, 480, 504, 528, 552, 576, 600, 624, 648, 672, 700]
DEFAULT_HEIGHTS = [24,32,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40,40]
...
def downsample_then_upsample(np_img: np.ndarray, min_scale=0.2, max_scale=0.6):
    ...
    h, w = np_img.shape
    scale = random.uniform(0.2, 0.40)
    print("Scale used: {}".format(scale))
...
def generate_one_image(text: str, fonts: List[str], out_size=(DEFAULT_MAX_WIDTH, DEFAULT_HEIGHT)):
    ...
    # Render text
    pil_img = render_text_as_image(text, fonts, base_height=out_size[1])
...
fonts = find_usable_fonts(DEFAULT_FONTS)
img = render_text_as_image("ш", fonts, base_height=32)
...
np_img = downsample_then_upsample(np_img, min_scale=0.15, max_scale=0.6)
np_img = resize_np_image(np_img, size=(10, 10))
np_img = brightness_contrast(np_img, brightness_delta=0.1, contrast_range=(0.6, 0.8))
np_img = np.clip(np_img, 0, 255).astype(np.uint8)

# img.save("test_image.png")
cv2.imwrite("test_image.png", np_img)
